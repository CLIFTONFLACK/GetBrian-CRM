...
} from "@/lib/actions/requirement-documents";

const DOC_TYPE_LABELS: Record<string, string> = {
  landlord_pack: "Landlord pack",
...
// client never needs it. (See src/lib/requirement-docs.ts.)
...
export function RequirementDocuments({
  ...
  const [docType, setDocType] = React.useState("landlord_pack");
  const [uploading, setUploading] = React.useState(false);
  const [error, setError] = React.useState<string | null>(null);

  // Uploads straight to Vercel Blob client-side, via a short-lived token from
  // src/app/api/blob/client-upload/route.ts (identical to the listing document
  // flow). The document stays effectively private: this component never
  // renders the raw Blob URL — `doc.url` below is the signed proxy link the
  // requirement page generated, not this upload's `blob.url`.
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const blob = await upload(path, file, {
        ...
        clientPayload: JSON.stringify({ kind: "requirement-doc", requirementId }),
      });
      ...
      const res = await addRequirementDocument({}, fd);
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-end gap-3 rounded-md border bg-muted/30 p-3">
        <div className="space-y-1">
          <Label htmlFor="req-doc-type" className="text-xs text-muted-foreground">
            Type
          </Label>
          <Select
            id="req-doc-type"
          ...
          >
            <option value="landlord_pack">Landlord pack</option>
            <option value="heads_of_terms">Heads of terms</option>
            <option value="brief">Brief</option>
            <option value="other">Other</option>
          </Select>
        </div>
        <div className="space-y-1">
          <Label htmlFor="req-doc-file" className="text-xs text-muted-foreground">
            Upload PDF
          </Label>
          <input
            id="req-doc-file"
